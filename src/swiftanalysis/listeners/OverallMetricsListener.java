 ...
 */
public class OverallMetricsListener extends SwiftBaseListener {
	...
	private static Map<String, Integer> optionalTypeMap = new HashMap<>();
	private static Map<String, Integer> forcedTypeMap = new HashMap<>();
	...
	private static Map<String, Integer> optionalChainingObjectCallDepthMap = new HashMap<>();
	...
	private static Map<String, Integer> optionalChainingCallDepthMap = new HashMap<>();

	// This map holds the amount catches present at a do block.
	private static Map<String, Integer> doCatchBlocksMap = new HashMap<>();
	private static Map<String, Integer> genericCatchBlockLengthMap = new HashMap<>();
	private static Map<String, Integer> catchBlockLengthMap = new HashMap<>();
	private static Map<String, Integer> catchErrorTypeMap = new HashMap<>();

	private static Set<String> printFunctionNames = new HashSet<>(Arrays.asList("print", "println", "NSLog"));

	public OverallMetricsListener(Printer printer) {
	...
	public void enterIfStatement(SwiftParser.IfStatementContext ctx) {
		...
		SwiftParser.ConditionListContext conditionClause = ctx.conditionList();
	...
	private boolean containsOnlyFunctionCalls(SwiftParser.CodeBlockContext catchBlock, Set<String> functionNames) {
	...
	private boolean isFunctionCall(SwiftParser.StatementContext statement, Set<String> functionNames) {
		SwiftParser.ExpressionContext expression = statement.expression();
		if (expression != null) {
			SwiftParser.PrefixExpressionContext prefix = expression.prefixExpression();
			if (prefix.getChildCount() == 1) {
				if (prefix.getChild(0) instanceof SwiftParser.FunctionCallExpressionContext) {
					SwiftParser.FunctionCallExpressionContext functionCall = (SwiftParser.FunctionCallExpressionContext) prefix.getChild(0);
	...
	public void enterVariableDeclaration(SwiftParser.VariableDeclarationContext ctx) {
		...
		if (ctx.patternInitializerList() != null && ctx.patternInitializerList().patternInitializer() != null){
			for (SwiftParser.PatternInitializerContext child : ctx.patternInitializerList().patternInitializer()) {
	 ...
	 * @param ctx The SwiftParser.ConditionListContext to be checked.
	...
	private static boolean containsNilCheck (ParserRuleContext ctx) {
		...
		return conditionClauseText.contains("==nil") ||
